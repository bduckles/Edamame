# Header ------------------------------------------------------------------
#Factor Levels for variables within a dataset and then plot to likert with factor 
...
# UseFactfunc/Datasame size -----------------------------------------------


#Use the Factor Function (factfunc) I just created and make the data all the same size
...
GrpLevel <- c("Post", "Pre")
Mlt_PerCompAbSc <- factfunc(Mlt_PerCompAbSc$Group, GrpLevel) %>% 
 as.data.frame() #error here "fs must be a list"
####### 
