...
source("Analysis2017_18/rename17.R")
...
#Titles and Likerts
PerCompTitle <- "2017 Edamame Perceived Computational Ability Scale"
PerCompAbilScLik <- likert(WOG_PerCompAb, grouping = J_PerCompAbilSc$Group)

CompUndTitle <- "2017 Edamame Computational Understanding Scale"
CompUndLik <- likert(WOG_CompUndSc, grouping = J_CompUndSc$Group)

CodeAbilTitle1 <- "2017 Edamame Coding Ability Scale, Chart 1"
CodeAbilLik1 <- likert(WOG_CodeAbilSc1, grouping = J_CodeAbilSc1$Group)

CodeAbilTitle2 <- "2017 Edamame Coding Ability Scale, Chart 2"
CodeAbilLik2 <- likert(WOG_CodeAbilSc2, grouping = J_CodeAbilSc2$Group)

ComfCompTitle <- "2017 Edamame Comfort with Computational Tasks Scale"
...
pdf("Charts/PerCompAbilScLikert17.pdf", width = 6, height = 10 )
...
pdf("Charts/CompUndLikert17.pdf", width = 6, height = 12)
plot(CompUndLik, colors=PurpleGreenFour) + ggtitle(CompUndTitle)
dev.off()

pdf("Charts/CodeAbilLikert1_17.pdf", width = 6, height = 11 )
plot(CodeAbilLik1, colors=PurpleGreenFour) + ggtitle(CodeAbilTitle1)
dev.off()

pdf("Charts/CodeAbilLikert2_17.pdf", width = 6, height = 10 )
plot(CodeAbilLik2, colors=PurpleGreenFour) + ggtitle(CodeAbilTitle2)
dev.off()

pdf("Charts/ComfComLikert17.pdf", width = 6, height = 10)
plot(ComfCompLik, 
     center = 3.5,
     plot.percent.neutral = FALSE,
